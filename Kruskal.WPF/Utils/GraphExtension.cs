...
public static class GraphExtension
{
        ...
        where T : IEquatable<T>
    {
        var defaultPenColor = Color.Black;
        var defaultBrushColor = Color.Green;
        var defaultPenWidth = 2;
        var edgePathPenColor = Color.Red;
        var edgePathPenWidth = 10;
        var bitmap = new Bitmap(width + 2 * margin, height + 2 * margin);

        using var g = Graphics.FromImage(bitmap);
        using var p = new Pen(defaultPenColor, defaultPenWidth);
        using var b = new SolidBrush(defaultBrushColor);
        using var f = new Font("Calibri", 14);

        var colors = new Random().RandomColors(graph.Edges.Count, [defaultPenColor, defaultBrushColor, Color.White]);
        var edgesPath = new List<Edge<T>>();
        for (int i = 0; i < path.path.Count - 1; i++)
        {
            var v1 = path.path[i];
            var v2 = path.path[i + 1];
            var edge = graph.EdgesDistinct.FirstOrDefault(e => e == new Edge<T>(v1, v2, 0))!;
            edgesPath.Add(edge);
        }
        ...
        for (int i = 0; i < graph.EdgesDistinct.Count; i++)
        {
            ...
            if (edgesPath.Contains(e))
            {
                p.Color = edgePathPenColor;
                p.Width = edgePathPenWidth;
            }
